...
std::vector<ElfW(Addr)> ElfImg::LinearRangeLookup(std::string_view name) const {
    ...
    for (auto [i, end] = symtabs_.equal_range(name); i != end; ++i) {
        ...
        LOGD("found %s "
                ...
                "%x"
            #endif
             " in %s in symtab by linear range lookup", name.data(), offset, elf.data());
    }
    return res;
}
...
ElfW(Addr) ElfImg::PrefixLookupFirst(std::string_view prefix) const {
    ...
    if (auto i = symtabs_.lower_bound(prefix); i != symtabs_.end() && i->first.starts_with(prefix)) {
        LOGD("found prefix %s of %s "
            #ifdef __aarch64__
                "%llx"
            #else
                "%x"
            #endif
             " in %s in symtab by linear lookup", prefix.data(), i->first.data(), i->second->st_value, elf.data());
        return i->second->st_value;
    } else {
        return 0;
    }
...
ElfImg::getSymbOffset(std::string_view name, uint32_t gnu_hash, uint32_t elf_hash) const {
    if (auto offset = GnuLookup(name, gnu_hash); offset > 0) {
        LOGD("found %s "
             ...
             "%x"
        #endif
             " in %s in dynsym by gnuhash", name.data(), offset, elf.data());
        return offset;
        ...
        LOGD("found %s "
             ...
             "%x"
        #endif
             " in %s in dynsym by elfhash", name.data(), offset, elf.data());
        return offset;
        ...
        LOGD("found %s "
             ...
             "%x"
        #endif
             " in %s in symtab by linear lookup", name.data(), offset, elf.data());
        return offset;
    } else {
        return 0;
    }
...
bool ElfImg::findModuleBase() {
    ...
    if (!found) {
        if (buff) free(buff);
        LOGE("failed to read load address for %p", elf.data());
        fclose(maps);
        return false;
    }
